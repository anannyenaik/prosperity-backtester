...
- `replay_<strategy>/`: full tick-level replay bundle for one script.
...
- `monte_carlo_<strategy>/`: targeted robustness bundle for a leading script.
...
Replay bundles populate Overview, Replay, product dives and Inspect. Comparison bundles populate Comparison. Round 2 scenario bundles populate Round 2 and aggregate ranking tables. Monte Carlo bundles populate Monte Carlo. Calibration bundles populate Calibration.
