...
HYDROGEL_SKEW_BASE = 0.045
HYDROGEL_SKEW_EXTRA = 0.035
...
class Trader:
    ...
    def _trade_hydrogel(self, state, memory, manager, mid):
        ...
        fair = base_fair - (HYDROGEL_SKEW_BASE + HYDROGEL_SKEW_EXTRA * cap_ratio) * position

        drift = abs(float(slow_ewma) - HYDROGEL_ANCHOR)
        trend_gap = abs(trend_signal)
        uncertainty_edge = min(4.0, 0.035 * max(0.0, drift - HYDROGEL_DRIFT_BAND) + 0.10 * max(0.0, trend_gap - HYDROGEL_TREND_BAND))
        take_edge = max(3.4, 0.25 * spread) + uncertainty_edge
        max_take = max(5, int(round(20 * size_scale))) if abs(position) < local_soft_cap else max(3, int(round(8 * size_scale)))
