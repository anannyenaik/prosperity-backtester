...
HYDROGEL_WARMUP_TICKS = 300
HYDROGEL_SOFT_CAP = 60
HYDROGEL_MIN_SOFT_CAP = 36
HYDROGEL_DRIFT_BAND = 22.0
HYDROGEL_DRIFT_LIMIT = 90.0
HYDROGEL_TREND_BAND = 3.5
HYDROGEL_TREND_LIMIT = 10.0
HYDROGEL_SKEW_BASE = 0.038
HYDROGEL_SKEW_EXTRA = 0.055
...
class Trader:
    ...
    def _hydrogel_fair(self, memory, mid):
        previous_slow = memory.get("h_slow")
        if previous_slow is None:
            slow_ewma = float(mid)
        else:
            slow_ewma = ewma(previous_slow, mid, HYDROGEL_SLOW_ALPHA) or float(previous_slow)

        previous_fast = memory.get("h_fast")
        if previous_fast is None:
            fast_ewma = float(mid)
        else:
            fast_ewma = ewma(previous_fast, mid, HYDROGEL_FAST_ALPHA) or float(previous_fast)

        tick = int(memory.get("tick", 0))
        warm = clamp(tick / float(HYDROGEL_WARMUP_TICKS), 0.0, 1.0)
        live_fair = 0.78 * slow_ewma + 0.14 * fast_ewma + 0.08 * float(mid)
        drift = abs(slow_ewma - HYDROGEL_ANCHOR)
        if drift <= HYDROGEL_DRIFT_BAND:
            drift_anchor_weight = 0.12
        elif drift >= HYDROGEL_DRIFT_LIMIT:
            drift_anchor_weight = 0.02
        else:
            ratio = (drift - HYDROGEL_DRIFT_BAND) / (HYDROGEL_DRIFT_LIMIT - HYDROGEL_DRIFT_BAND)
            drift_anchor_weight = 0.12 - 0.10 * ratio
        anchor_weight = (1.0 - warm) * 0.28 + warm * drift_anchor_weight
        base_fair = live_fair + anchor_weight * (HYDROGEL_ANCHOR - live_fair)
        return base_fair, slow_ewma, fast_ewma

    def _hydrogel_size_scale(self, memory, slow_ewma, fast_ewma):
        tick = int(memory.get("tick", 0))
        warm = clamp(tick / float(HYDROGEL_WARMUP_TICKS), 0.0, 1.0)
        warm_scale = 0.35 + 0.65 * warm
        drift = abs(float(slow_ewma) - HYDROGEL_ANCHOR)
        ...
        if warm >= 0.95 and drift > HYDROGEL_DRIFT_BAND:
            if trend_gap <= HYDROGEL_TREND_BAND:
                drift_scale = max(drift_scale, 0.72)
            elif trend_gap <= 2.0 * HYDROGEL_TREND_BAND:
                drift_scale = max(drift_scale, 0.60)
        ...
        if trend_gap <= HYDROGEL_TREND_BAND:
            ...
            ratio = (trend_gap - HYDROGEL_TREND_BAND) / (HYDROGEL_TREND_LIMIT - HYDROGEL_TREND_BAND)
            trend_scale = 1.0 - 0.55 * ratio
        return clamp(warm_scale * drift_scale * trend_scale, 0.30, 1.0)
    ...
    def _trade_hydrogel(self, state, memory, manager, mid):
        ...
        base_fair, slow_ewma, fast_ewma = self._hydrogel_fair(memory, mid)
        size_scale = self._hydrogel_size_scale(memory, slow_ewma, fast_ewma)
        local_soft_cap = max(HYDROGEL_MIN_SOFT_CAP, int(round(HYDROGEL_SOFT_CAP * size_scale)))
        ...
        cap_ratio = min(1.0, abs(position) / max(1.0, float(local_soft_cap)))
        fair = base_fair - (HYDROGEL_SKEW_BASE + HYDROGEL_SKEW_EXTRA * cap_ratio * cap_ratio) * position

        drift = abs(float(slow_ewma) - HYDROGEL_ANCHOR)
        trend_gap = abs(trend_signal)
        drift_uncertainty = max(0.0, drift - HYDROGEL_DRIFT_BAND)
        if trend_gap <= HYDROGEL_TREND_BAND:
            drift_uncertainty *= 0.35
        uncertainty_edge = min(4.0, 0.035 * drift_uncertainty + 0.10 * max(0.0, trend_gap - HYDROGEL_TREND_BAND))
        take_edge = max(3.4, 0.25 * spread) + uncertainty_edge
        take_base = 20
        max_take = max(5, int(round(take_base * size_scale))) if abs(position) < local_soft_cap else max(3, int(round(8 * size_scale)))
        for ask in sorted(depth.sell_orders):
            if ask > fair - take_edge:
                break
            if trend_block_buy and position >= 0:
                break
            if position >= local_soft_cap and ask >= base_fair:
                break
        ...
        for bid in sorted(depth.buy_orders, reverse=True):
            ...
            if trend_block_sell and position <= 0:
                break
            if position <= -local_soft_cap and bid <= base_fair:
                break
        ...
        if bid is None or ask is None:
            return
        quote_size = max(2, int(round(7 * size_scale))) if abs(manager.position(HYDROGEL)) < local_soft_cap else max(1, int(round(3 * size_scale)))
        passive_edge = max(5.2, 0.38 * spread) + 0.5 * uncertainty_edge
        bid_price = min(bid + 1, math.floor(fair - passive_edge))
        ask_price = max(ask - 1, math.ceil(fair + passive_edge))
        position = manager.position(HYDROGEL)
        if bid_price < ask and position < local_soft_cap and not (trend_block_buy and position >= 0):
            manager.add(HYDROGEL, bid_price, quote_size)
        if ask_price > bid and position > -local_soft_cap and not (trend_block_sell and position <= 0):
    ...
    def _update_state(self, memory, model, mids):
        ...
        if h_mid is not None:
            ...
            fast_seed = memory.get("h_fast")
            memory["h_slow"] = round(float(h_mid) if slow_seed is None else ewma(slow_seed, h_mid, HYDROGEL_SLOW_ALPHA), 4)
            memory["h_fast"] = round(float(h_mid) if fast_seed is None else ewma(fast_seed, h_mid, HYDROGEL_FAST_ALPHA), 4)
        else:
            memory["h_slow"] = round(float(memory.get("h_slow", HYDROGEL_ANCHOR)), 4)
            memory["h_fast"] = round(float(memory.get("h_fast", HYDROGEL_ANCHOR)), 4)
