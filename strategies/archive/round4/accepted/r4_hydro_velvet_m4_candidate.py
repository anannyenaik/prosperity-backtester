# Round 4, active candidate: Hydro v2 + VELVET M4 overlay (frozen).
# - HYDROGEL_PACK and voucher logic UNCHANGED from the archived accepted
#   Hydro v2 reference.
# - VELVET runs the baseline z-score leg PLUS a premium-cross-only overlay
#   (M4 from Phase 3): spread==1, abs(ask_vol)<15, cross at best ask, size 8,
#   long soft cap 100, no broad crossing, no passive, no name modifiers.
# - Optional baseline-VELVET position clip (PARAMS["VELVET_BASELINE_CAP"], None
#   = no clip = full baseline). Reduced-cap values 150 and 100 are documented
#   fallbacks only; they are not the active default.
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "HYDROGEL_PACK":
                ...
                if product == "VELVETFRUIT_EXTRACT":
                    # Optional baseline-VELVET position clip (anchor-risk
                    # variant). Reshapes z-score orders so resulting position
                    # cannot exceed +/-cap, but never blocks an order that
                    # *reduces* exposure.
                    ...
                    if cap is not None:
                        ...
                        for o in orders:
                            ...
                            if q > 0:
                                ...
                                # If position is already at +cap, only allow buys that reduce a short.
                                if proj >= 0:
