...
#   M2_stop_add_near_cap       M3 upper cap plus selected central/deep stop-add.
#   M3_asymmetric_strike_caps  M3 upper cap plus selected one-sided strike caps.
#   M4_net_delta_add_gate      M3 upper cap plus voucher net-delta add clipping.
#   M6_terminal_stop_add_or_reduce
#                               M3 upper cap plus late central/deep stop-add.
#   M7_combined_best_surgical  M3 upper cap plus at most two selected hardeners.
#
# All modes share:
...
# r4_hydro_velvet_m4_candidate.py and r4_trader.py are NEVER modified.
#
...
VOUCHER_RISK = {
    ...
    "central_deep_stop_add_products": [],
    "central_deep_stop_add_level": 275,
    "central_deep_side_caps": {},
    "central_deep_net_delta_cap": 1500,
    "central_deep_net_delta_priority": [5300, 5200, 5100, 5000, 4500, 4000, 5500, 5400],
    "terminal_start_fraction": 0.90,
    "terminal_stop_add_products": [],
    "terminal_stop_add_level": 275,
    "combined_use_side_caps": False,
    "combined_use_stop_add": False,
    "combined_use_terminal_stop": False,
}
...
def _clip_inventory_band_orders(orders, position, min_position=None, max_position=None):
    """Clip orders so projected inventory remains inside optional bounds."""
    expected_pos = int(position)
    lo = None if min_position is None else int(min_position)
    hi = None if max_position is None else int(max_position)
    out = []
    for order in orders:
        qty = int(order.quantity)
        if qty > 0 and hi is not None:
            room = hi - expected_pos
            if room <= 0:
                continue
            qty = min(qty, room)
        elif qty < 0 and lo is not None:
            room = expected_pos - lo
            if room <= 0:
                continue
            qty = -min(-qty, room)
        if qty != 0:
...
def _stop_add_near_abs_cap_orders(orders, position, near_cap):
    """Allow risk reduction, but stop orders extending exposure beyond near_cap."""
    cap = int(near_cap)
    expected_pos = int(position)
    out = []
    for order in orders:
        ...
        if qty > 0:
            if expected_pos >= cap:
                continue
            if expected_pos + qty > cap:
                qty = cap - expected_pos
        elif qty < 0:
            if expected_pos <= -cap:
                continue
            if expected_pos + qty < -cap:
...
def _side_cap_bounds(product, side_caps):
    if not isinstance(side_caps, dict):
        return None, None
    K = get_voucher_strike(product)
    cfg = side_caps.get(product)
    if cfg is None and K is not None:
        cfg = side_caps.get(str(int(K)))
    if not isinstance(cfg, dict):
        return None, None

    long_cap = cfg.get("long")
    if long_cap is None:
        long_cap = cfg.get("positive")
    short_cap = cfg.get("short")
    if short_cap is None:
        short_cap = cfg.get("negative")

    min_position = None if short_cap is None else -abs(int(short_cap))
    max_position = None if long_cap is None else abs(int(long_cap))
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        risk_modes = {
            ...
            "M2_stop_add_near_cap",
            "M3_asymmetric_strike_caps",
            "M4_net_delta_add_gate",
            "M6_terminal_stop_add_or_reduce",
            "M7_combined_best_surgical",
        }
        ...
        if risk_handled:
            ...
            def _apply_m3_upper_cap():
                for upper_product in ("VEV_5400", "VEV_5500"):
                    risk_orders_by_product[upper_product] = _clip_positive_inventory_orders(
                        risk_orders_by_product.get(upper_product, []),
                        state.position.get(upper_product, 0),
                        200,
                    )

            def _apply_central_deep_stop_add(products_key, level_key):
                selected = set(VOUCHER_RISK.get(products_key, []) or [])
                if not selected:
                    return
                level = int(VOUCHER_RISK[level_key])
                for selected_product in selected:
                    risk_orders_by_product[selected_product] = _stop_add_near_abs_cap_orders(
                        risk_orders_by_product.get(selected_product, []),
                        state.position.get(selected_product, 0),
                        level,
                    )

            def _apply_central_deep_side_caps():
                side_caps = VOUCHER_RISK.get("central_deep_side_caps", {})
                for capped_product in tuple(f"VEV_{K}" for K in (4000, 4500, 5000, 5100, 5200, 5300)):
                    min_position, max_position = _side_cap_bounds(capped_product, side_caps)
                    if min_position is None and max_position is None:
                        continue
                    risk_orders_by_product[capped_product] = _clip_inventory_band_orders(
                        risk_orders_by_product.get(capped_product, []),
                        state.position.get(capped_product, 0),
                        min_position,
                        max_position,
                    )

            def _apply_central_deep_net_delta_gate():
                return _apply_net_delta_soft_cap(
                    risk_orders_by_product,
                    _projected_net_delta({k: 0 for k in _BS_STRIKE_LIST}),
                    bs_deltas,
                    int(VOUCHER_RISK["central_deep_net_delta_cap"]),
                    VOUCHER_RISK["central_deep_net_delta_priority"],
                )
            ...
            elif risk_mode == "M2_stop_add_near_cap":
                _apply_m3_upper_cap()
                _apply_central_deep_stop_add(
                    "central_deep_stop_add_products",
                    "central_deep_stop_add_level",
                )

            elif risk_mode == "M3_asymmetric_strike_caps":
                _apply_m3_upper_cap()
                _apply_central_deep_side_caps()

            elif risk_mode == "M4_net_delta_add_gate":
                _apply_m3_upper_cap()
                risk_orders_by_product = _apply_central_deep_net_delta_gate()

            elif risk_mode == "M6_terminal_stop_add_or_reduce":
                _apply_m3_upper_cap()
                if float(ts) / 1_000_000.0 >= float(VOUCHER_RISK["terminal_start_fraction"]):
                    _apply_central_deep_stop_add(
                        "terminal_stop_add_products",
                        "terminal_stop_add_level",
                    )

            elif risk_mode == "M7_combined_best_surgical":
                _apply_m3_upper_cap()
                if VOUCHER_RISK.get("combined_use_side_caps", False):
                    _apply_central_deep_side_caps()
                if VOUCHER_RISK.get("combined_use_stop_add", False):
                    _apply_central_deep_stop_add(
                        "central_deep_stop_add_products",
                        "central_deep_stop_add_level",
                    )
                if VOUCHER_RISK.get("combined_use_terminal_stop", False):
                    if float(ts) / 1_000_000.0 >= float(VOUCHER_RISK["terminal_start_fraction"]):
                        _apply_central_deep_stop_add(
                            "terminal_stop_add_products",
                            "terminal_stop_add_level",
