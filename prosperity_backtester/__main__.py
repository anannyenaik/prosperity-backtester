...
def build_parser() -> argparse.ArgumentParser:
    ...
    mc = sub.add_parser(
        ...
        epilog=(
            ...
            "  python -m prosperity_backtester monte-carlo strategies/trader.py --sessions 256 --workers 4 --mc-backend rust\n"
        ),
    )
    add_shared(mc)
    ...
    mc.add_argument("--mc-backend", default="auto", choices=["auto", "classic", "streaming", "rust"], help="Monte Carlo execution backend. 'auto' always resolves to 'streaming' (fastest default). 'classic' materialises full synthetic market days per session (highest fidelity, slowest). 'streaming' runs the hot loop directly in Python with no IPC overhead and is the strongest measured choice through the tracked 8-worker benchmark cases. 'rust' offloads market generation and order execution to a compiled Rayon engine via a Python subprocess per worker; it remains available for explicit ceiling experiments, but is not auto-selected and is currently slower than streaming on the tracked benchmark fixture. The Rust binary is built with cargo the first time (~60s, cached after that).")
