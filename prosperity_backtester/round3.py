        ...
        for symbol in ROUND3_VOUCHERS:
            ...
            for timestamp in day_dataset.timestamps:
                ...
                if option_snapshot.mid is None:
                    continue
                mids.append(float(option_snapshot.mid))
                if underlying_snapshot is None or underlying_snapshot.mid is None:
                    continue
                spot = float(underlying_snapshot.mid)
                intrinsic = intrinsic_value(spot, strikes)
                time_val = time_value(option_snapshot.mid, spot, strikes)
                intrinsic_values.append(intrinsic)
                if time_val is not None:
                    time_values.append(time_val)
                iv = implied_vol_bisection(option_snapshot.mid, spot, strikes, t_years)
                if iv is None:
                    continue
                ivs.append(iv)
                delta = call_delta(spot, strikes, t_years, iv)
            ...
            diagnostics = VoucherDiagnostics(
                ...
                average_mid=_mean(mids),
                average_spread=_mean(spreads),
                average_intrinsic=_mean(intrinsic_values),
                average_time_value=_mean(time_values),
                iv_mean=_mean(ivs),
                iv_p05=_quantile(ivs, 0.05),
                iv_p95=_quantile(ivs, 0.95),
                iv_std=statistics.pstdev(ivs) if len(ivs) > 1 else 0.0 if ivs else None,
                delta_mean=_mean(deltas),
                valid_iv_count=len(ivs),
                observation_count=len(mids),
                include_in_surface_fit=bool(meta.include_in_surface_fit),
                warnings=tuple(_strike_warnings(meta, mids, ivs)),
            )
    ...
    *,
    ...
    for day_row in diagnostics.get("days", []):
        for voucher in day_row.get("vouchers", []):
            if not voucher.get("include_in_surface_fit"):
                continue
            iv_mean = voucher.get("iv_mean")
            strike = voucher.get("strike")
            if iv_mean is None or strike is None:
                continue
            by_strike.setdefault(int(strike), []).append(float(iv_mean))
    if not by_strike:
        return {parse_voucher_symbol(symbol): 0.25 for symbol in ROUND3_SURFACE_FIT_VOUCHERS}
    surface = {
...
def prepare_round3_synthetic_context(
    ...
    *,
    ...
    diagnostics = compute_option_diagnostics(market_days, round_spec=spec)
    surface = robust_surface_iv_by_strike(market_days, round_spec=spec)
